...
# ---- Draw ----
fig, ax = plt.subplots(figsize=(8.0, 9.0), dpi=200)
...
ellipse(10.5, 13.0, 3.2, 1.8, "Internal\nPolitical\nEfficacy")
ellipse(10.5, 6.0,  3.2, 1.8, "External\nPolitical\nEfficacy")

internal_items = [(f"D1.1_{i}", 17 - (i - 1) * 1.0) for i in range(1, 9)]
external_items = [(f"D2.1_{i}", 10 - (i - 1) * 1.0) for i in range(1, 9)]

for it, y in internal_items:
    rect(2.5, y, 1.7, 0.55, it)
    ax.annotate("", xy=(8.95, 13.0), xytext=(3.35, y),
                arrowprops=dict(arrowstyle="-|>", color=MID, lw=1.0))
    t = 0.7
    midx = 3.35 + t * (8.95 - 3.35); midy = y + t * (13.0 - y)
...
for it, y in external_items:
    rect(2.5, y, 1.7, 0.55, it)
    ax.annotate("", xy=(8.95, 6.0), xytext=(3.35, y),
                arrowprops=dict(arrowstyle="-|>", color=MID, lw=1.0))
    t = 0.7
    midx = 3.35 + t * (8.95 - 3.35); midy = y + t * (6.0 - y)
    ax.text(midx, midy + 0.18, f"{std[it]:.2f}", fontsize=7.5, color=DEEP,
            ha="center", style="italic", fontweight="bold")

ax.annotate("", xy=(10.5, 6.95), xytext=(10.5, 12.05),
            arrowprops=dict(arrowstyle="<->", color=ORANGE, lw=2.0,
                            connectionstyle="arc3,rad=0.55"))
ax.text(13.3, 9.5, f"r = {fcor:.2f}", fontsize=11, color=ORANGE,
        fontweight="bold", style="italic")

ax.text(7, 18.5, "Confirmatory Factor Analysis: Two-Factor Efficacy Model",
